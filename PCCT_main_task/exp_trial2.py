...
def submit_association():
    ...
    if tasks[task_index][1] == '联想':
        ...
        if len(words) == 10:
            ...
            path_len, clustering_coefficient = analyze_modified_network(words) #立刻计算语义网络的拓扑属性，如果只收数据就不用
...
def show_examp_sele():
    ...
    self_rate = rating_var.get()
    result.append(self_rate)
    display_label.config(text=f"请问您的答案是否和他人的答案相关联，如果是，请输入该'答案'，如果否，请填写‘没有相关答案’", fg="black",font=("Times",30))
    entry.pack(pady=20)
    entry.focus_set()
    submit_sele_button.pack(pady=20)
